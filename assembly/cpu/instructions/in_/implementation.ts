...
export function inc_(inst: instruction.Instruction, effective: u32) : bool {
	switch (inst.step - instruction.firstStep) {
		...
		case 1:
			...
			if (is8Bit) {
				const result: u8 = inc_u8(bus.read.fetch());
...
export function inc_acc(inst: instruction.Instruction) : bool {
	if (flags.E || flags.M) {
		registers.A = inc_u8(registers.A);
	}

	else {
		registers.C = inc_u16(registers.C);
	}
...
export function inx_(inst: instruction.Instruction) : true {
	if (flags.E || flags.X) {
		registers.X_low = inc_u8(registers.X_low);
	}

	else {
		registers.X = inc_u16(registers.X);
	}
...
export function iny_(inst: instruction.Instruction) : true {
	if (flags.E || flags.X) {
		registers.Y_low = inc_u8(registers.Y_low);
	}

	else {
		registers.Y = inc_u16(registers.Y);
	}
...
function inc_u16(operand: u16) : u16 {
