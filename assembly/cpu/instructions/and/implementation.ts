...
export function and(inst: instruction.Instruction, effective: u32) : bool {
	switch (inst.step - instruction.firstStep) {
		...
		case 1:
			...
			if (is8Bit) {
				return and_u8(inst, bus.read.fetch());
			}
...
// @ts-ignore: decorator
@inline export function and_u8(inst: instruction.Instruction, operand: u8) : true {
	registers.A &= operand;
...
// @ts-ignore: decorator
@inline export function and_u16(inst: instruction.Instruction, operand: u16) : true {
	registers.C &= operand;
